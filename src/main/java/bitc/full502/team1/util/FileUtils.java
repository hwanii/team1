package bitc.full502.team1.util;
...
//                가져온 파일 정보를 FileDTO 객체에 저장
                ...
                fileDTO.setStoredFileName(path + "/" + newFileName); // 위에서 생성한 전체 경로 + 현재시간을 기준으로 생성한 새 파일명을 합하여 전체 경로가 있는 파일명을 생성
                fileDTO.setCreateId(createId); // 사용자 ID

//                반환할 파일 리스트에 FileDTO 객체를 저장
                fileList.add(fileDTO);

//                File 객체를 전체 경로 + 현재시간 기준 새파일명으로 객체 생성
                file = new File(path + "/" + newFileName);
//                지정한 전체 경로로 업로드된 파일을 복사함
